...
VAR
  ...
  cl,cf:VecRecord;
BEGIN
  FOR i:=0 TO SR.mdl.Count-1 DO BEGIN
    IF SphereClass(SR.mdl[i]).isLight THEN BEGIN
      tVert:=SR.GenLight(i);
      LP.Clear;
      LP.Add(tVert);
...
//OMEGA 算出
        IF obj.REFL=DIFF THEN BEGIN
          sw:=r.d;
          tr:=VecSQR(SphereClass(SR.mdl[tVert.id]).p-x);
          tr:=SphereClass(SR.mdl[tVert.id]).rad2/tr;
          ts:=sw*cV.n;
          IF ts<0 THEN ts:=-ts;
          IF tr>1 THEN BEGIN
